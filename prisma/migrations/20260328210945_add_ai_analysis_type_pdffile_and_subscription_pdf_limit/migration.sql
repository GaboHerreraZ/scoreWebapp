...
VALUES
  ('aiAnalysisType', 'estudioCredito', 'Estudio de crédito', 'Análisis IA del resultado de un estudio de crédito', true, 1, NOW(), NOW()),
  ('aiAnalysisType', 'cargaPdfEstadosFinancieros', 'Carga PDF estados financieros', 'Extracción de datos financieros desde PDF con IA', true, 2, NOW(), NOW())
...
-- Backfill existing rows with 'estudioCredito' parameter id
UPDATE "ai_analyses"
SET "type_id" = (SELECT "id" FROM "parameters" WHERE "type" = 'aiAnalysisType' AND "code" = 'estudioCredito' LIMIT 1)
